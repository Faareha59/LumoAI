...
const NavItem: React.FC<{ icon: React.ReactNode; label: string; isActive: boolean; onClick: () => void; comingSoon?: boolean }> = ({ icon, label, isActive, onClick, comingSoon }) => (
  <button
    onClick={comingSoon ? undefined : onClick}
    className={`w-full flex items-center justify-between gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-colors ${isActive ? 'bg-foreground text-background' : 'text-muted-foreground hover:bg-card hover:text-foreground'} ${comingSoon ? 'cursor-not-allowed opacity-60' : ''}`}
  >
    ...
    {comingSoon && (
      <span className="text-[8px] font-black uppercase tracking-widest bg-black/5 text-black px-1.5 py-0.5 rounded border border-black/10">Soon</span>
    )}
  </button>
);
...
const Sidebar: React.FC<SidebarProps> = ({ user, currentView, setView, onLogout }) => {
  const studentNav = [
    ...
    { view: 'marketplace' as AppView, label: 'Skill Swap', icon: <SparklesIcon className="w-5 h-5" /> },
